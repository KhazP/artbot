...
function containsInquiryOnlyIndicators(text) {
    return /price on request|inquire|iletişime geçiniz|fiyat sorunuz|fiyat istek üzerine verilir|daha fazla bilgi için lütfen bize ulaşın/i.test(text);
}
...
function extractJsonLdScripts(content) {
    const scripts = [];
...
function collectValueByKeys(node, keys) {
...
function matchFirst(scriptContent, patterns) {
...
function captureMatchValue(match) {
    if (!match) {
        return "";
    }
    return (match[1] ?? match[2] ?? "").trim();
}
function extractScriptPayloadFields(content) {
    const fallback = {
        title: null,
        artistName: null,
        medium: null,
    ...
    const lotNumber = matchFirst(scriptBlob, [
...
function estimateMidpoint(low, high) {
    if (low !== null && high !== null)
        return (low + high) / 2;
    return low ?? high;
}
export function parseGenericLotFields(content, baseUrl) {
    ...
    const estMatch = text.match(/(?:estimate|estimated|estimate range|tahmini|ekspertiz)\s*[:\-]?\s*([\d.,\s₺$€A-Za-z]+)\s*(?:-|to|–|—)\s*([\d.,\s₺$€A-Za-z]+)/i);
    const realizedMatch = text.match(/(?:realized|sold for|satış fiyatı|satildi|satıldı|çekiç|cekic|hammer)\s*[:\-]?\s*([\d.,\s₺$€A-Za-z]+)/i);
    const premiumMatch = text.match(/(?:buyers?\s*premium|buyer's premium|alıcı primi|alici primi)\s*(?:included|dahil|hariç|haric)?/i);
    const askMatch = text.match(/(?:asking price|buy now|hemen al|price|fiyat)\s*[:\-]?\s*([\d.,\s₺$€A-Za-z]+)/i);
    const inquiryOnly = containsInquiryOnlyIndicators(text);
    let priceType = jsonLd.priceType !== "unknown" ? jsonLd.priceType : scriptPayload.priceType;
    let priceAmount = jsonLd.priceAmount ?? scriptPayload.priceAmount;
    let estimateLow = jsonLd.estimateLow ?? scriptPayload.estimateLow;
    let estimateHigh = jsonLd.estimateHigh ?? scriptPayload.estimateHigh;
    let buyersPremiumIncluded = null;
    if (estMatch) {
        priceType = "estimate";
    ...
    if (realizedMatch) {
        ...
        if (realizedAmount !== null && realizedAmount > 0) {
            ...
            else if (/buyers?\s*premium|buyer's premium|alıcı primi|alici primi/.test(lowerText) &&
    ...
    const hasExplicitPriceSignal = Boolean(estMatch) ||
    ...
    const dimensionsMatch = text.match(/(?:dimensions?|ölçüler?|olculer|boyut(?:lar)?)\s*[:\-]?\s*([0-9.,\sx×*]+(?:cm|mm|in)?(?:\s*x\s*[0-9.,\sx×*]+(?:cm|mm|in)?)*)/i);
...
export function parseLotFields(input) {
    const html = input.html ?? "";
    const markdown = input.markdown ?? "";
    return parseGenericLotFields(`${html}\n${markdown}`);
}
//# sourceMappingURL=lot-parser.js.map